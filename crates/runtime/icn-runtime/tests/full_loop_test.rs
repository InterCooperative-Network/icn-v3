...
async fn test_mana_regeneration_policy_from_config() -> anyhow::Result<()> {
    ...
    let temp_dir = tempdir()?; // For storage_path
    let config = RuntimeConfig {
        node_did: "test-node-did-for-config-test".to_string(), // Required by RuntimeConfig
        storage_path: temp_dir.path().to_path_buf(),        // Required by from_config for SledStorage
        ...
        // For example, if key_path is None, from_config generates one.
        // If other services URLs are None, Noop versions might be used.
        ..Default::default() 
    };

    // 2. Construct Runtime using from_config.
    // This will use the policy from the config.
    // Runtime::from_config is async.
    let runtime = Runtime::<InMemoryManaLedger>::from_config(config).await?;

    // 3. Get the ledger from the runtime (it was created by from_config).
    // Ensure the mana_regenerator and its ledger are present.
    ...
    let ledger_from_runtime = regenerator_opt.unwrap().ledger.clone();
    
    // 4. Set initial state on this ledger.
    ledger_from_runtime.set_initial_state(test_user_did.clone(), ManaState {
    ...
    info!("Spawning runtime for mana regeneration (from_config) test...");
    let runtime_handle = tokio::spawn(runtime.run_forever());

    // 6. Wait long enough for at least one regeneration tick.
    let tick_interval_secs = 30; // As defined in run_forever
    ...
    info!("Test woke up, checking mana state (from_config)...");

    // 7. Assert on the ledger.
    let updated_state_option = ledger_from_runtime.get_mana_state(&test_user_did).await?;
    assert!(updated_state_option.is_some(), "ManaState should exist for the DID (from_config)");

    let updated_state = updated_state_option.unwrap();
    assert_eq!(
        updated_state.current_mana, expected_mana_after_tick,
        "Mana should have regenerated by {} units as per config", regeneration_amount
    );

    info!(
        "Mana regeneration (from_config) test successful. Final mana: {}. Expected: {}",
        updated_state.current_mana, expected_mana_after_tick
    );
