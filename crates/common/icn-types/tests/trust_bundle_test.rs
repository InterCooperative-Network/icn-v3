...
fn test_trust_bundle_verification() {
    ...
    assert!(result.is_ok(), "Bundle verification with threshold failed: {:?}", result);
    ...
    assert!(result.is_ok(), "Bundle verification with weights failed: {:?}", result);
