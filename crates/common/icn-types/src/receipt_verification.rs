...
mod tests {
    ...
    fn verify_missing_signature_fails() {
        ...
        let result = receipt.verify_signature();
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Receipt signature is missing"));
    }
    ...
    fn verify_empty_issuer_did_fails() {
        ...
        let result = receipt.verify_signature();
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Receipt issuer DID is empty"));
    }
    ...
    fn verify_malformed_issuer_did_fails() {
        ...
        let result = receipt.verify_signature();
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Invalid issuer DID format"));
    }
    ...
    fn verify_malformed_signature_bytes_fails() {
        ...
        let result = receipt.verify_signature();
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Invalid signature byte format"));
    }
} 