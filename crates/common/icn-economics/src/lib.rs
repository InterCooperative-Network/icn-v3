...
pub trait ResourceRepository {
    ...
    async fn get_usage_history(&self, 
        did: &Did,
        resource_type: &str, 
        scope: &str, 
        since_timestamp: u64
...
impl ResourcePolicyEnforcer {
    ...
    pub fn set_policy(&mut self, resource_type: &str, scope: &str, policy: ResourceAuthorization) {
        self.policies.insert((resource_type.to_string(), scope.to_string()), policy);
    }
    ...
    pub fn get_policy(&self, resource_type: &str, scope: &str) -> Option<&ResourceAuthorization> {
        self.policies.get(&(resource_type.to_string(), scope.to_string()))
    }
}
...
impl PolicyEnforcer for ResourcePolicyEnforcer {
    async fn check_authorization(&self, did: &Did, token: &ScopedResourceToken) -> Result<bool> {
        ...
        let policy = self.get_policy(&token.resource_type, &token.scope)
            .ok_or_else(|| anyhow!("No policy found for resource type {} in scope {}", 
                      token.resource_type, token.scope))?;
        ...
        if let Some(expires_at) = token.expires_at {
            ...
            if now > expires_at {
                return Err(EconomicsError::InvalidToken(
                    format!("Token expired at {}, current time is {}", expires_at, now)
                ).into());
            }
        }
        ...
        match policy {
            ...
            ResourceAuthorization::Quota(quota) => {
                ...
                let usage = self.repository.get_usage(did, &token.resource_type, &token.scope).await?;
                
                if usage + token.amount <= *quota {
                    Ok(true)
                } else {
                    Err(EconomicsError::QuotaExceeded(
                        format!("Quota of {} exceeded (usage: {}, requested: {})", 
                              quota, usage, token.amount)
                    ).into())
                }
            },
            
            ResourceAuthorization::RateLimit { amount, period_secs } => {
                ...
                let usage_history = self.repository
                ...
                if total_usage + token.amount <= *amount {
                    ...
                    Err(EconomicsError::RateLimitExceeded(
                        format!("Rate limit of {} per {} seconds exceeded (usage: {}, requested: {})",
            ...
            ResourceAuthorization::PermitList(permits) => {
                ...
                if permits.contains(&did.to_string()) {
                    ...
                    Err(EconomicsError::AccessDenied(
                        format!("DID {} not in permit list for resource type {} in scope {}",
...
impl ResourceRepository for InMemoryResourceRepository {
    async fn record_usage(&self, did: &Did, token: &ScopedResourceToken) -> Result<()> {
        let key = (did.to_string(), token.resource_type.clone(), token.scope.clone());
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .map_err(|e| anyhow!("Error getting current time: {}", e))?
            .as_secs();
    ...
    async fn get_usage_history(&self, 
        ...
        let filtered = self.usage.get(&key)
            .map(|records| {
                records.iter()
...
mod tests {
    ...
    async fn test_permit_list_policy() {
        ...
        enforcer.set_policy("admin", "test-scope", ResourceAuthorization::PermitList(
            vec![did1.to_string()]
        ));
