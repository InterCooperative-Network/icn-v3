...
impl Economics {
    pub fn new(policy: ResourceAuthorizationPolicy) -> Self {
        Self { policy }
    }
    ...
    pub fn authorize(
        ...
        debug!(
            "Authorizing {} units of {:?} for {} (coop: {:?}, community: {:?})",
        ...
        if self.policy.authorized(rt, amt) {
            ...
            debug!(
                "Authorization denied for {} to use {} units of {:?}",
                caller, amt, rt
            );
            -1
        }
    ...
    pub async fn record(
        ...
        debug!(
            "Recording {} units of {:?} for {} (coop: {:?}, community: {:?})",
            amt, rt, caller, coop_id, community_id
        );
    ...
    pub async fn mint(
        ...
        debug!(
            "Minting {} tokens for {} (coop: {:?}, community: {:?})",
        ...
        debug!(
            "New token balance for {}: {} tokens (usage: {})",
            recipient, available_tokens, *current
        );
        0
    }
    ...
    pub async fn transfer(
        ...
        if args.rt != ResourceType::Token {
            debug!(
                "Attempted to transfer non-token resource type: {:?}",
                args.rt
            );
            return -3;
        }
        ...
        debug!(
            ...
            args.amt, args.sender, args.recipient
        );
        let mut l = ledger.write().await;
        ...
        debug!(
            "Transfer complete. New balances: {} usage={}, {} usage={}",
            args.sender,
            sender_usage + args.amt,
            args.recipient,
            new_recipient_usage
        );
        0 // Success
        ...
        l.iter()
            .filter(|(k, _)| {
                k.resource_type == rt
                    && k.coop_id
                        .as_ref()
                        .is_some_and(|cid| cid == &coop_id.to_string())
            })
            .map(|(_, v)| *v)
            .sum()
    }
        ...
        l.iter()
            .filter(|(k, _)| {
                k.resource_type == rt
                    && k.community_id
                        .as_ref()
                        .is_some_and(|cid| cid == &community_id.to_string())
            })
            .map(|(_, v)| *v)
            .sum()
    }
