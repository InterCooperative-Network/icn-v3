...
async fn authorize_ok_and_record() {
    ...
    assert_eq!(
        econ.record(&did, None, None, ResourceType::Token, 10, &ledger)
            .await,
        0
    );
...
async fn authorize_fail() {
    let econ = Economics::new(ResourceAuthorizationPolicy {
        token_allowance: 5,
        ..Default::default()
    });
    let did = KeyPair::generate().did;
    assert_eq!(
        econ.authorize(&did, None, None, ResourceType::Token, 10),
        -1
    );
}
...
async fn test_transfer_success() {
    ...
    // Verify balances after transfer
    {
        ...
        assert_eq!(
            *l.get(&sender_key).unwrap_or(&0),
            40,
            "Sender should have usage of 40"
        );
        ...
        assert_eq!(
            *l.get(&recipient_key).unwrap_or(&100),
            0,
            "Recipient should have usage of 0"
        );
    }
}
...
async fn test_transfer_insufficient_funds() {
    ...
    // Verify balances unchanged
    {
        ...
        assert_eq!(
            *l.get(&sender_key).unwrap_or(&0),
            80,
            "Sender should still have usage of 80"
        );
        ...
        assert!(
            l.get(&recipient_key).is_none(),
            "Recipient should have no entry"
        );
    }
}
