...
async fn test_scoped_resource_usage() {
    ...
    assert_eq!(
        usage, 100,
        "Usage within the same organization scope should be 100"
    );
    ...
    let usage = economics
        .get_usage(&did, None, None, ResourceType::Cpu, &ledger)
        .await;
    ...
    economics
        .record(&did, Some(&coop_id), None, ResourceType::Cpu, 75, &ledger)
        .await;

    // Get usage for coop only scope - should be 75
    let usage = economics
        .get_usage(&did, Some(&coop_id), None, ResourceType::Cpu, &ledger)
        .await;
    assert_eq!(usage, 75, "Usage within coop-only scope should be 75");

    // Test cooperative usage aggregation
    let coop_usage = economics
        .get_cooperative_usage(&coop_id, ResourceType::Cpu, &ledger)
        .await;
    assert_eq!(
        coop_usage, 225,
        "Total cooperative usage should be 225 (150 + 75)"
    );
}
...
async fn test_scoped_token_operations() {
    ...
    let user1_coop_balance = economics
        .get_usage(&user1, Some(&coop_id), None, ResourceType::Token, &ledger)
        .await;
    assert_eq!(
        user1_coop_balance, 0,
        "User1's coop balance should show 0 usage (has 100 tokens)"
    );

    let user1_community_balance = economics
        .get_usage(
            &user1,
            Some(&coop_id),
            Some(&community_id),
        ...
        .await;
    assert_eq!(
        user1_community_balance, 0,
        "User1's community balance should show 0 usage (has 50 tokens)"
    );
    ...
    let user1_coop_balance = economics
        ...
        .await;
    assert_eq!(
        user1_coop_balance, 30,
        "User1's coop balance should show 30 usage (has 70 tokens)"
    );

    let user2_coop_balance = economics
        .get_usage(&user2, Some(&coop_id), None, ResourceType::Token, &ledger)
        .await;
    assert_eq!(
        user2_coop_balance, 0,
        "User2's coop balance should show 0 usage (has 30 tokens)"
    );
    ...
    let user1_community_balance = economics
        ...
        .await;
    assert_eq!(
        user1_community_balance, 20,
        "User1's community balance should show 20 usage (has 30 tokens)"
    );

    let user2_community_balance = economics
        .get_usage(
            &user2,
            Some(&coop_id),
            Some(&community_id),
        ...
        .await;
    assert_eq!(
        user2_community_balance, 0,
        "User2's community balance should show 0 usage (has 20 tokens)"
    );

    // Verify organization-wide token distribution
    let coop_token_usage = economics
        .get_cooperative_usage(&coop_id, ResourceType::Token, &ledger)
        .await;
    assert_eq!(
        coop_token_usage, 50,
        "Total cooperative token usage should be 50"
    );

    let community_token_usage = economics
        .get_community_usage(&community_id, ResourceType::Token, &ledger)
        .await;
    assert_eq!(
        community_token_usage, 20,
        "Total community token usage should be 20"
    );
}
