...
fn test_jws_sign_verify_roundtrip() {
    ...
    assert!(tamper_result.is_err(), "Verification should fail with tampered payload");
    ...
    assert!(tamper_sig_result.is_err(), "Verification should fail with tampered signature");
} 