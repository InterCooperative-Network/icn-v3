...
fn test_credential_canonical_serialization() {
    ...
    let verification_method = format!("did:key:test#{}", base64::encode(&keypair.public.to_bytes()[..8]));
    ...
    let canonical_bytes = credential.canonical_bytes().expect("Canonicalization failed");
    
    // The proof should be excluded from the canonical bytes
    let canonical_json: serde_json::Value = serde_json::from_slice(&canonical_bytes)
        .expect("Failed to parse canonical JSON");
    
    assert!(canonical_json.get("proof").is_none(), "Proof should be excluded from canonical bytes");
    
    // Check if the canonical bytes are deterministic
    let canonical_bytes2 = credential.canonical_bytes().expect("Canonicalization failed");
    assert_eq!(canonical_bytes, canonical_bytes2, "Canonical bytes should be deterministic");
...
fn test_credential_signing_and_verification() {
    ...
    assert!(result.is_ok(), "Verification of signed credential failed: {:?}", result);
