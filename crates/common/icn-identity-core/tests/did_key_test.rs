...
fn test_did_key_roundtrip() {
    ...
    // Verify format
    assert!(did_key.starts_with("did:key:z"), "DID key should start with did:key:z");
    ...
    assert_eq!(extracted_pk, public_key, "Extracted public key should match original");
}
...
fn test_did_key_rfc_vector() {
    ...
    assert_eq!(did_key, expected_did, "Generated DID key should match RFC test vector");
