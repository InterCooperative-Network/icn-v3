...
fn test_receipt_with_org_identifiers() {
    ...
    assert_eq!(
        receipt.community_id.as_ref().unwrap().to_string(),
        "community-456"
    );
...
fn test_cid_changes_with_different_orgs() {
    ...
    assert_ne!(
        cid2, cid3,
        "Receipt with community ID should have different CID"
    );
    assert_ne!(
        cid1, cid3,
        "Receipts with different org scopes should have different CIDs"
    );
}
