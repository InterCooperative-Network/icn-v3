...
fn get_receipt_signing_payload(receipt: &ExecutionReceipt) -> Result<Vec<u8>, SignError> {
    ...
    serde_cbor::to_vec(&receipt_clone)
        .map_err(|e| SignError::Serialization(format!("Failed to serialize receipt for payload: {}", e)))
}
...
pub fn sign_receipt_in_place(receipt: &mut ExecutionReceipt, kp: &KeyPair) -> Result<(), SignError> {
    ...
    if receipt.executor != kp.did {
        return Err(SignError::InvalidSignature(
            format!("KeyPair DID '{}' does not match receipt executor DID '{}'", kp.did, receipt.executor)
        ));
    }
...
pub fn verify_embedded_signature(receipt: &ExecutionReceipt) -> Result<bool, SignError> {
    if receipt.signature.is_empty() {
        return Err(SignError::InvalidSignature("Receipt has no signature to verify.".to_string()));
    }
...
mod tests {
    ...
    fn test_sign_and_verify_receipt_in_place() {
        ...
        assert!(!receipt.signature.is_empty(), "Signature should not be empty after signing");
        ...
        assert!(is_tampered_valid.is_err() || !is_tampered_valid.unwrap(), "Verification of tampered signature should fail or error");
        ...
        assert!(sign_mismatch_result.is_err(), "Signing with mismatched KeyPair DID and executor DID should fail");

        // Sign correctly with its own keypair
        sign_receipt_in_place(&mut receipt_for_other_kp, &another_kp).expect("Signing with correct keypair failed");
        ...
        let is_valid_other = verify_embedded_signature(&receipt_for_other_kp).expect("Verification of other receipt failed");
        assert!(is_valid_other, "Verification of correctly signed other receipt should succeed");
    }
    ...
    fn test_verify_empty_signature() {
        ...
        match verification_result.unwrap_err() {
            SignError::InvalidSignature(msg) => assert!(msg.contains("Receipt has no signature to verify")),
