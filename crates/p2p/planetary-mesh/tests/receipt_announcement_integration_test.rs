...
use icn_types::mesh::{JobId as IcnJobId, MeshJob, MeshJobParams};
...
use icn_types::dag_store::{DagNode, DagStore, DagStoreError, ListResult, SearchUniqueResult};
...
impl DagStore for MockDagStore {
    async fn insert(&self, _node: DagNode) -> Result<Cid, DagStoreError> {
        if self.fail_on_insert {
            Err(DagStoreError::Other(
                "Simulated DAG insert failure".to_string(),
            ))
        } else {
            // Return a dummy CID if we were to allow success
            Ok(
                Cid::try_from("bafybeiczssuxtmccagh2h5cy6kvhax2t5h5yaijhc4yrk4h6svoqy3zlce")
                    .unwrap(),
            )
        }
    }
    ...
    ) -> Result<ListResult, DagStoreError> {
        Ok(ListResult {
            nodes: vec![],
            total: 0,
            limit: 0,
...
async fn test_execution_receipt_announcement_roundtrip() {
    ...
    let mesh_job = MeshJob {
        ...
        params: MeshJobParams {
            wasm_cid: "bafybeigdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef"
                .to_string(),
            ..Default::default()
        },
    ...
    {
        assert_eq!(
            received_job_id, job_id_str,
            "Job ID mismatch in announcement"
        );
        assert_eq!(
            announcer_did, executor_did,
            "Executor DID mismatch in announcement"
        );
        ...
        panic!(
            "Unexpected message type received via broadcast channel: {:?}",
            received_message
        );
    }
}
...
async fn test_receipt_announcement_anchoring_failure() {
    ...
    assert!(
        trigger_result.is_ok(),
        "Execution trigger should succeed even if anchoring fails internally: {:?}",
        trigger_result.err()
    );
    ...
    assert!(
        announce_result.is_ok(),
        "Timeout waiting for potential receipt announcement"
    );
    let received_message_res = announce_result.unwrap();
    assert!(
        received_message_res.is_ok(),
        "Broadcast receive failed while expecting announcement"
    );
    ...
    } = received_message_res.unwrap()
    {
        assert_eq!(
            received_job_id, job_id_str,
            "Job ID mismatch in announcement (anchor fail)"
        );
        assert_eq!(
            announcer_did, executor_did,
            "Executor DID mismatch in announcement (anchor fail)"
        );
        assert!(
            !received_receipt_cid_str.is_empty(),
            "Received receipt CID should not be empty (anchor fail)"
        );
    } else {
        panic!(
            "Unexpected message type received (anchor fail): {:?}",
            received_message_res
        );
    }
    ...
    let completed_cids_guard = completed_receipt_cids_store
        .read()
        .expect("Failed to lock completed_job_receipt_cids");
    assert!(
