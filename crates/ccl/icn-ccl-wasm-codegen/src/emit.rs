...
use wasm_encoder::{Module, Function, Instruction, ValType, CodeSection, TypeSection, ImportSection, EntityType, FunctionSection};
...
pub fn program_to_wasm(prog: &Program) -> Vec<u8> {
    ...
    for (_op_idx, op) in prog.ops.iter().enumerate() {
        ...
        match op {
            ...
            Opcode::MintToken { res_type, amount, recipient, data } => {
            ...
            Opcode::SetProperty { key, value_json,.. } => {
    ...
    type_section.function(vec![ValType::I32, ValType::I64, ValType::I32, ValType::I32], vec![]); // 3: mint_token(res: ptr, amt: i64, recip: ptr, data: ptr)
